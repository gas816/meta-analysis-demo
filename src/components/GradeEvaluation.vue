  ...
  <div class="grade-container">
    ...
    <div class="toolbar">
      <div class="tool-select">
        ...
        <el-select v-model="store.grade.tool" placeholder="选择工具" style="width: 200px">
    ...
    <div class="main-content">
      ...
      <div class="outcome-list">
        ...
        >
          <div class="outcome-name">{{ profile.outcomeName || '未命名结局' }}</div>
          <el-tag size="small" :type="getLevelTagType(profile.overallCertainty)">
      ...
      <div class="evaluation-panel" v-if="currentProfile">
        <div class="panel-header">
          ...
          <div class="actions">
            <el-button type="success" @click="autoEvaluate" :loading="evaluating">
        ...
        <el-form label-position="top" class="grade-form">
            ...
            <el-col :span="8">
              <el-form-item label="重要性">
                <el-select v-model="currentProfile.importance">
                  ...
                  <el-option label="不重要 (Not important)" value="Not important" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="4">
              <el-form-item label="纳入研究数">
                <el-input-number v-model="currentProfile.numberOfStudies" :min="0" />
              </el-form-item>
            </el-col>
            <el-col :span="4">
          ...
          <el-divider content-position="left">降级因素 (Downgrade Factors)</el-divider>

          <!-- 5个降级因素 -->
          <div class="factor-grid">
            <div class="factor-card" v-for="factor in downgradeFactors" :key="factor.key">
          ...
          <el-divider content-position="left">升级因素 (Upgrade Factors)</el-divider>

          <el-checkbox-group v-model="upgradeFactorsSelection" @change="handleUpgradeChange">
            <el-checkbox label="largeEffect">效应量大 (Large Effect, RR>2 or <0.5)</el-checkbox>
            <el-checkbox label="doseResponse">剂量反应关系 (Dose Response)</el-checkbox>
            <el-checkbox label="confounding">混杂因素可能减弱效应 (Confounding)</el-checkbox>
          </el-checkbox-group>
          ...
          <div class="result-section">
            <div class="final-grade">
              <span>最终证据等级：</span>
              <el-tag :type="getLevelTagType(currentProfile.overallCertainty)" size="large" effect="dark">
    ...
    <!-- SoF 表格弹窗 -->
    <el-dialog v-model="showSofTable" title="Summary of Findings (SoF) Table" width="80%">
        ...
        <el-table-column label="Certainty of the Evidence (GRADE)" width="250">
          <template #default="{ row }">
            <div class="grade-cell">
              <el-tag :type="getLevelTagType(row.overallCertainty)" size="small">{{ row.overallCertainty }}</el-tag>
              <div class="grade-icons">
                <span v-if="row.riskOfBias.rating !== 'not-serious'" title="Risk of Bias">⚠️RoB</span>
                <span v-if="row.inconsistency.rating !== 'not-serious'" title="Inconsistency">⚠️Inc</span>
                <span v-if="row.indirectness.rating !== 'not-serious'" title="Indirectness">⚠️Ind</span>
                <span v-if="row.imprecision.rating !== 'not-serious'" title="Imprecision">⚠️Imp</span>
                <span v-if="row.publicationBias.rating !== 'undetected'" title="Pub Bias">⚠️Pub</span>
              </div>
            </div>
          </template>
        </el-table-column>
...
const downgradeFactors = [
  ...
  { 
    ...
    options: [
      ...
      { label: '强烈怀疑 (Strongly suspected, -2)', value: 'strongly-suspected' }
    ]
  }
] as const;
...
const addOutcome = () => {
  const newProfile: GradeProfile = {
    ...
    importance: 'Critical',
    numberOfStudies: store.literatures.filter(l => l.status === 'included').length,
...
const recalculateGrade = () => {
  ...
  // 降级
  const downgradeMap: Record<string, number> = { 'not-serious': 0, 'serious': 1, 'very-serious': 2, 'undetected': 0, 'suspected': 1, 'strongly-suspected': 2 };
